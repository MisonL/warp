...
fn classify_returns_server_unavailable_error_when_list_unavailable() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            ...
            let err = classify_agent_mode_base_model_id("claude-sonnet-4-5", &valid_ids, true, ctx)
...
fn classify_returns_unknown_id_error_when_list_available_and_id_genuinely_invalid() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            ...
            let err =
                classify_agent_mode_base_model_id("claude-sonnet-4-5", &valid_ids, false, ctx)
...
fn classify_accepts_id_in_choices_even_when_list_unavailable() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            ...
            let id = classify_agent_mode_base_model_id("custom-config-key", &valid_ids, true, ctx)
