...
fn localized_environment_action(locale: LocaleId, action: &str) -> String {
    match action {
        "create" => text_for_locale(locale, "agent_sdk.environment.action.create"),
        "delete" => text_for_locale(locale, "agent_sdk.environment.action.delete"),
        "update" => text_for_locale(locale, "agent_sdk.environment.action.update"),
        _ => action.to_owned(),
    }
}
...
impl EnvironmentCommandRunner {
    ...
    fn get(&mut self, id: String, ctx: &mut ModelContext<Self>) {
        ...
        ctx.spawn(initial_sync, move |_, result, ctx| {
            ...
            if let Some(environment) = environment {
                Self::print_environment_details(&environment.model().string_model, locale);
                ctx.terminate_app(warpui::platform::TerminationMode::ForceTerminate, None);
            } else {
                ctx.terminate_app(
                    warpui::platform::TerminationMode::ForceTerminate,
    ...
    fn print_environment_details(env: &AmbientAgentEnvironment, locale: LocaleId) {
        println!(
            "{}",
        ...
        match &env.base_image {
            ...
            None => {
                println!(
                    "{}",
                    text_for_locale(locale, "agent_sdk.environment.detail.docker_image_none")
                );
            }
        }
        if env.github_repos.is_empty() {
            println!(
                "{}",
                text_for_locale(locale, "agent_sdk.environment.detail.repositories_none")
            );
        } else {
            println!(
                "{}",
                text_for_locale(locale, "agent_sdk.environment.detail.repositories")
            );
        ...
        if env.setup_commands.is_empty() {
            println!(
                "{}",
                text_for_locale(locale, "agent_sdk.environment.detail.setup_commands_none")
            );
        } else {
            println!(
                "{}",
                text_for_locale(locale, "agent_sdk.environment.detail.setup_commands")
            );
            ...
            InquireError::OperationCanceled | InquireError::OperationInterrupted => {
                eprintln!(
                    ...
                    localization::text_for_app(
                        ctx,
                        "agent_sdk.environment.output.creation_canceled"
                    )
                );
                ctx.terminate_app(warpui::platform::TerminationMode::ForceTerminate, None);
                true
            ...
            Ok(response) => match response.list_warp_dev_images {
                ListWarpDevImagesResult::ListWarpDevImagesOutput(output) => {
                    ...
                    let custom_image_option =
                        localization::text_for_app(ctx, "agent_sdk.environment.custom_image");
        ...
        F: FnOnce(&mut ModelContext<Self>) + Send + 'static,
    {
        ...
        let locale = localization::current_locale(ctx);
        let operation = localized_environment_action(locale, operation_name);
                        ...
                        match status.status {
                            ...
                            UserRepoAuthStatusEnum::UserNotConnectedToGithub => {
                                eprintln!(
                                    ...
                                    text_for_locale(
                                        locale,
                                        "agent_sdk.environment.auth.github_not_connected"
                                    )
                                );
                                has_blocking_private_issues = true;
                                break;
                    ...
                    match (response.auth_url, response.tx_id) {
                        (Some(auth_url), Some(tx_id)) => {
                            ...
                            println!(
                                ...
                                text_for_locale(
                                    locale,
                                    "agent_sdk.environment.auth.private_repo_required"
                                )
                            );
                            println!(
                                "{}\n",
                        ...
                        (Some(auth_url), None) => {
                            ...
                            println!(
                                ...
                                text_for_locale(
                                    locale,
                                    "agent_sdk.environment.auth.rerun_after_authorizing"
                                )
                            );
                            ctx.terminate_app(
                                warpui::platform::TerminationMode::ForceTerminate,
                        ...
                        (None, Some(_)) => {
                            ...
                            ctx.terminate_app(
                                ...
                                Some(Err(anyhow::anyhow!(text_for_locale(
                                    locale,
                                    "agent_sdk.environment.error.oauth_missing_auth_url"
                                )))),
                            );
                        }
                        (None, None) => {
                ...
                Err(e) => {
                    ctx.terminate_app(
                        ...
                        Some(Err(e.context(text_for_locale(
                            locale,
                            "agent_sdk.environment.error.github_auth_status",
                        )))),
                    );
                }
            }
        ...
        F: FnOnce(&mut ModelContext<Self>) + Send + 'static,
    {
        let integrations_client = ServerApiProvider::as_ref(ctx).get_integrations_client();
        let locale = localization::current_locale(ctx);
        let localized_action = localized_environment_action(locale, action);
            ...
            move |_, result, ctx| match result {
                Ok(output) => {
                    if !output.provider_names.is_empty() {
                        ...
                        let prompt_message = text_for_locale_with_args(
                            locale,
    ...
    fn update_environment(
        ...
        ctx.spawn(initial_sync, move |_, result, ctx| {
            ...
            let server_id = match ServerId::try_from(id.as_str()) {
                Ok(sid) => sid,
                Err(_) => {
                    let error = anyhow::anyhow!(localization::text_for_app_with_args(
                        ctx,
                        "agent_sdk.environment.error.not_found",
                        &[("id", &id)],
                    ));
...
impl TableFormat for EnvironmentInfo {
    fn header() -> Vec<Cell> {
        environment_header_for_locale(LocaleId::EnUs)
    }

    fn header_for_locale(locale: LocaleId) -> Vec<Cell> {
        environment_header_for_locale(locale)
    }
...
fn environment_header_for_locale(locale: LocaleId) -> Vec<Cell> {
    vec![
        Cell::new(text_for_locale(locale, "agent_sdk.environment.table.id")),
        Cell::new(text_for_locale(locale, "agent_sdk.environment.table.name")),
        ...
        Cell::new(text_for_locale(locale, "agent_sdk.environment.table.scope")),
    ]
}
...
impl TableFormat for ImageInfo {
    fn header() -> Vec<Cell> {
        image_header_for_locale(LocaleId::EnUs)
    }

    fn header_for_locale(locale: LocaleId) -> Vec<Cell> {
        image_header_for_locale(locale)
    }
...
fn image_header_for_locale(locale: LocaleId) -> Vec<Cell> {
    vec![
        Cell::new(text_for_locale(
            locale,
