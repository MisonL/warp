...
fn always_safe_boundary() -> BoundaryCheck {
    Arc::new(|| Box::pin(async { Boundary::Safe }))
}

fn initializing_boundary() -> BoundaryCheck {
    Arc::new(|| Box::pin(async { Boundary::Initializing }))
}
...
impl RecordingClient {
    ...
    fn pending_upload_targets(server_base_url: String) -> Arc<Self> {
        Arc::new(Self {
            server_base_url,
            http: build_test_http_client(),
            commit_delay: Duration::ZERO,
            fail_commit: false,
            upload_targets_pending: true,
            upload_calls: AtomicUsize::new(0),
            commit_calls: AtomicUsize::new(0),
            commit_notify: Arc::new(Notify::new()),
        })
    ...
    async fn wait_for_commit_count(&self, expected: usize, timeout: Duration) -> bool {
        tokio::time::timeout(timeout, async {
            loop {
                let notified = self.commit_notify.notified();
                if self.commit_calls.load(Ordering::SeqCst) >= expected {
                    break;
                }
                notified.await;
            }
...
impl HarnessSupportClient for RecordingClient {
    ...
    async fn get_snapshot_upload_targets(
        ...
        if self.upload_targets_pending {
            return std::future::pending::<Result<Vec<UploadTarget>>>().await;
        }
...
fn finalize_budget_exceeds_the_attempt_floor_for_the_driver_defaults() {
    ...
    assert!(
        finalize_budget(script, upload) > DECLARATIONS_WRITER_FLUSH_TIMEOUT + script + upload,
        "the budget callers derive must exceed the full floor `finalize_with_new_attempt` enforces"
    );
    assert!(
...
async fn finalize_with_new_attempt_below_floor_skips_the_attempt_entirely() {
    ...
    let boundary_check = always_safe_boundary();
    let (ack_tx, ack_rx) = oneshot::channel();
    let request = FinalizeRequest {
        deadline: Instant::now() + Duration::from_millis(50),
        ack: ack_tx,
    };
    // With no writer, floor = script_timeout + upload_timeout = 2s, comfortably above
    // the 50ms budget.
...
#[tokio::test]
async fn finalize_with_new_attempt_uploads_after_setup_failure() {
    let client = FailingUploadTargetsClient::new();
    let task_id = fresh_task_id();
    let tempdir = TempDir::new().unwrap();
    let file_path = tempdir.path().join("note.txt");
    std::fs::write(&file_path, b"setup left a workspace change").unwrap();
    let _decl = DeclarationsFixture::new(&task_id, &file_path);
    let boundary_check = initializing_boundary();
    let (ack_tx, ack_rx) = oneshot::channel();
    let request = FinalizeRequest {
    ...
    assert_eq!(
        client.call_count.load(Ordering::SeqCst),
        1,
        "cleanup must retain the final snapshot even when setup failed before the checkpoint gate"
    );
    assert!(ack_rx.await.is_ok(), "ack must still be sent");
}
...
#[tokio::test]
async fn finalize_with_in_flight_attempt_returns_after_budget_elapses() {
    ...
    let finalization = tokio::spawn(finalize_with_in_flight_attempt(request, result_rx));

    tokio::time::timeout(Duration::from_secs(1), &mut ack_rx)
        .await
        .expect("finalization must respect its shutdown deadline")
        .expect("finalization must always ack");
    finalization.await.expect("finalization task should finish");
    assert!(
        result_tx.send(CheckpointResult::Skipped).is_err(),
        "the result receiver should be released once the shutdown budget expires"
    );
}
...
#[tokio::test]
async fn checkpoint_attempt_timeout_preserves_generation_for_retry() {
    let task_id = fresh_task_id();
    let tempdir = TempDir::new().unwrap();
    let file_path = tempdir.path().join("note.txt");
    std::fs::write(&file_path, b"hi").unwrap();
    let _decl = DeclarationsFixture::new(&task_id, &file_path);
    let client = RecordingClient::pending_upload_targets(String::new());
    ...
    let CheckpointResult::Failed {
        generation: Some(generation),
        reason,
    } = result
    else {
        panic!("an upload timeout must retain the minted generation, got {result:?}");
    };
    assert!(
        reason.contains("upload timeout"),
        "expected the timeout failure, got: {reason}"
    );
...
#[tokio::test]
async fn coordinator_waits_for_conversation_before_checkpointing() {
    let client = FailingUploadTargetsClient::new();
    let task_id = fresh_task_id();
    let tempdir = TempDir::new().unwrap();
    let file_path = tempdir.path().join("note.txt");
    ...
    let conversation_ready = Arc::new(AtomicUsize::new(0));
    let conversation_ready_for_boundary = conversation_ready.clone();
    let boundary_check: BoundaryCheck = Arc::new(move || {
        let conversation_ready = conversation_ready_for_boundary.clone();
        Box::pin(async move {
            if conversation_ready.load(Ordering::SeqCst) == 0 {
    ...
    assert_eq!(
        client.call_count.load(Ordering::SeqCst),
        0,
        "setup must not trigger a checkpoint before the conversation exists"
    );

    conversation_ready.store(1, Ordering::SeqCst);
    wait_for(
        || client.call_count.load(Ordering::SeqCst) >= 1,
        Duration::from_secs(2),
    )
    ...
    // Stop the coordinator without starting another final attempt.
...
async fn coordinator_returns_to_idle_and_waits_a_full_interval_before_the_next_attempt() {
    ...
    assert!(
        !client.wait_for_commit_count(2, interval / 4).await,
...
async fn coordinator_finalize_awaits_the_in_flight_attempt_without_a_second_commit() {
    ...
    // Wait for the commit call itself rather than polling: under the full parallel module suite
    // scheduler contention can exceed a short wall-clock polling window before the task runs.
    assert!(
        client
            .wait_for_commit_count(1, Duration::from_secs(10))
            .await,
        "the periodic attempt must enter commit_snapshot before finalization"
    );
...
async fn coordinator_stops_when_the_driver_is_gone() {
    ...
    let boundary_calls = Arc::new(AtomicUsize::new(0));
    let boundary_calls_for_closure = boundary_calls.clone();
    let boundary_notify = Arc::new(Notify::new());
    let boundary_notify_for_closure = boundary_notify.clone();
    let driver_gone: BoundaryCheck = Arc::new(move || {
        let calls = boundary_calls_for_closure.clone();
        let notify = boundary_notify_for_closure.clone();
        Box::pin(async move {
    ...
    let first_boundary = boundary_notify.notified();
    ...
    tokio::time::timeout(Duration::from_secs(5), first_boundary)
        .await
        .expect("the coordinator must check the boundary once");
    // Well past several intervals: the loop must have returned rather than kept ticking.
    assert!(
        tokio::time::timeout(Duration::from_millis(300), boundary_notify.notified())
            .await
            .is_err(),
        "a gone driver must stop the loop rather than checking the boundary again"
    );
    assert_eq!(
...
async fn coordinator_stops_running_attempts_after_finalize() {
    ...
    assert!(
        !client
            .wait_for_commit_count(after_finalize + 1, interval * 10)
            .await,
...
async fn coordinator_finalize_from_idle_skips_attempt_below_floor() {
    ...
    // floor = 10s; a 100ms budget is well below it (and would also be below the
    // writer-inclusive floor).
    handle.finalize(Duration::from_millis(100)).await;
