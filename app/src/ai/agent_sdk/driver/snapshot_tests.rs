...
impl TestClient {
    ...
    /// Drops `drop_trailing` targets from the *first* upload-targets call only, leaving
    /// later chunks (and therefore the manifest) intact.
    fn new_dropping_trailing_on_first_call(
        server_base_url: String,
        drop_trailing: usize,
    ) -> Arc<Self> {
        Arc::new(Self {
...
impl HarnessSupportClient for TestClient {
    ...
    async fn get_snapshot_upload_targets(
        ...
        let call_index = {
            let mut recorded = self.upload_requests.lock().unwrap();
        ...
        let mut targets: Vec<UploadTarget> = request
            ...
            .collect();
        let drop_count = if self.drop_trailing_first_call_only && call_index > 0 {
            0
        } else {
            self.drop_trailing_targets
        };
        let keep = targets.len().saturating_sub(drop_count);
...
#[cfg(unix)]
#[tokio::test]
async fn declarations_script_command_is_killed_when_timeout_drops_output_future() {
    let tempdir = snaptest_tempdir();
    let script_path = tempdir.path().join("declarations.sh");
    let marker_path = tempdir.path().join("marker");
    fs::write(
        &script_path,
        "#!/bin/sh\nprintf started > \"$1\"\nsleep 2\nprintf done >> \"$1\"\n",
    )
    ...
    let declarations_path = tempdir.path().join("declarations.jsonl");
    let mut command = declarations_script_command(&script_path, tempdir.path(), &declarations_path);
    command.arg(&marker_path);
    let result = tokio::time::timeout(Duration::from_millis(100), command.output()).await;
    assert!(result.is_err(), "the script should exceed the test timeout");

    // A surviving shell would write `done` after its sleep. Give it enough time to do so before
    // checking the marker, which makes the assertion detect a missing kill_on_drop setting.
    tokio::time::sleep(Duration::from_millis(2500)).await;
    let contents = fs::read_to_string(&marker_path).expect("script should write its start marker");
    assert!(contents.contains("started"));
    assert!(
        !contents.contains("done"),
        "timed-out declarations script survived and continued writing: {contents:?}"
    );
}
...
fn checkpoint_withholds_commit_when_a_required_blob_fails() {
    ...
    assert!(
        reason.contains("required entries"),
        "expected the required-entry failure reason specifically, got: {reason}"
    );
    assert!(
...
#[test]
fn checkpoint_withholds_commit_when_a_declared_file_cannot_be_read() {
    let tempdir = snaptest_tempdir();
    // A deleted path is intentionally stale and gets filtered before gather. Use a directory
    // instead: metadata succeeds, but reading it as a file still produces a real read failure.
    let unreadable_path = tempdir.path().join("not-a-file");
    fs::create_dir(&unreadable_path).unwrap();
    let decl_dir = snaptest_tempdir();
    let declarations_path = write_declarations(decl_dir.path(), &[], &[&unreadable_path]);
    ...
    let CheckpointResult::Failed { reason, .. } = result else {
        panic!("a read failure must fail the checkpoint, got {result:?}");
    };
    assert!(
        reason.contains("read"),
        "expected the read-failure reason specifically, got: {reason}"
    );
    assert!(
        client.commit_requests().is_empty(),
        "commit must be withheld when a declared file cannot be read"
    );
    manifest_mock.assert();
}

#[test]
fn checkpoint_ignores_deleted_declarations_but_keeps_existing_files() {
    let tempdir = snaptest_tempdir();
    let existing_file = tempdir.path().join("still-there.txt");
    let deleted_file = tempdir.path().join("deleted.txt");
    fs::write(&existing_file, b"keep me").unwrap();
    fs::write(&deleted_file, b"remove me").unwrap();
    let decl_dir = snaptest_tempdir();
    let declarations_path =
        write_declarations(decl_dir.path(), &[], &[&existing_file, &deleted_file]);
    fs::remove_file(&deleted_file).unwrap();

    let mut server = Server::new();
    let existing_mock = server
        .mock("PUT", upload_path("still-there\\.txt"))
        .with_status(200)
        .expect(1)
        .create();
    let manifest_mock = server
    ...
    let committed = client.commit_requests();
    assert_eq!(committed.len(), 1);
    assert!(
        committed[0]
            .files
            .iter()
            .any(|file| file == "still-there.txt")
    );
    assert!(
        !committed[0]
            .files
            .iter()
            .any(|file| file.contains("deleted"))
    );
    existing_mock.assert();
    manifest_mock.assert();
}

#[test]
fn checkpoint_withholds_commit_when_a_repo_cannot_be_gathered() {
    let repo_dir = snaptest_tempdir();
    let decl_dir = snaptest_tempdir();
    let declarations_path = write_declarations(decl_dir.path(), &[repo_dir.path()], &[]);
    ...
    let CheckpointResult::Failed { reason, .. } = result else {
        panic!("a gather failure must fail the checkpoint, got {result:?}");
    };
    assert!(
        reason.contains("gather"),
        "expected the gather-failure reason specifically, got: {reason}"
    );
    assert!(
        client.commit_requests().is_empty(),
        "commit must be withheld when a repository cannot be gathered"
    );
    manifest_mock.assert();
}
...
fn checkpoint_manifest_upload_failure_withholds_commit() {
    ...
    assert!(
        ...
        "expected the manifest-failure reason specifically, got: {reason}"
    );
    assert!(
        client.commit_requests().is_empty(),
        "commit must be withheld when the manifest fails to upload"
    );
    manifest_mock.assert();
    drop(file_mock);
}
...
#[test]
fn checkpoint_target_allocation_failure_skips_commit() {
    // The server refuses to allocate upload targets at all. No blobs or manifest are ever
    // uploaded, and commit must never be attempted.
    let tempdir = snaptest_tempdir();
    let file_path = tempdir.path().join("note.txt");
    fs::write(&file_path, b"hello").unwrap();
    let decl_dir = snaptest_tempdir();
    ...
    assert!(
        reason.contains("allocate"),
        "expected the target-allocation-failure reason specifically, got: {reason}"
    );
    assert!(
        client.commit_requests().is_empty(),
        "commit must never be attempted when upload targets can't be allocated"
    );
}
...
fn checkpoint_withholds_commit_when_the_server_omits_a_blob_upload_target() {
    ...
    let client = TestClient::new_dropping_trailing_on_first_call(server.url(), 1);
    let result = Runtime::new()
        .unwrap()
        .block_on(run_checkpoint_from_declarations_file(
            &declarations_path,
    ...
    let CheckpointResult::Failed { reason, .. } = result else {
        panic!("a blob without an upload target must fail the attempt, got {result:?}");
    };
    assert!(
        reason.contains("upload target"),
        "expected the missing-target reason specifically, got: {reason}"
    );
    assert!(
        client.commit_requests().is_empty(),
        "commit must be withheld when the server omitted a blob's upload target"
    );
    drop(upload_mock);
}
...
fn sanitize_name_component_never_yields_the_reserved_double_underscore() {
    for raw in [
        ...
        assert!(
            ...
            "sanitized {raw:?} left characters outside the server charset: {sanitized}"
        );
