...
use crate::ai::local_harness_setup::{
    local_harness_is_product_enabled, local_harness_setup_message_key, local_harness_setup_state,
    LocalHarnessSetupState,
};
use crate::ai::orchestration::{
    accept_disabled_reason_with_auth as canonical_accept_disabled_reason_with_auth,
    empty_env_recommendation_message as canonical_empty_env_recommendation_message,
...
fn localized_validation_message(message: &str, app: &AppContext) -> String {
    if let Some(key) = local_harness_setup_message_key(message) {
    ...
    key.map(|key| localization::text_for_app(app, key))
...
pub fn accept_disabled_reason_with_auth(
    ...
    canonical_accept_disabled_reason_with_auth(state, app)
        .map(|message| localized_validation_message(&message, app))
}
...
pub fn empty_env_recommendation_message(
    ...
    canonical_empty_env_recommendation_message(execution_mode, app)
        .map(|message| localized_validation_message(&message, app))
}
    ...
    dropdown.update(ctx, |dropdown, ctx_dropdown| {
        ...
        match harness {
            ...
            Some(Harness::Codex) if is_local => {
                ...
                let items = vec![default_model_menu_item::<A>(&default_label)];
                dropdown.set_rich_items(items, ctx_dropdown);
                dropdown.set_selected_by_name(&default_label, ctx_dropdown);
            }
            Some(harness) => {
                ...
                let mut items: Vec<MenuItem<DropdownAction>> =
                    vec![default_model_menu_item::<A>(&default_label)];
                ...
                let selected_display_name = if initial_model_id.is_empty() {
                    ...
                    availability
                        ...
                        .or_else(|| Some(default_label.clone()))
                };
                dropdown.set_rich_items(items, ctx_dropdown);
                if let Some(name) = &selected_display_name {
                    dropdown.set_selected_by_name(name, ctx_dropdown);
            ...
            if selectable {
                ...
                let tooltip = match local_setup_state {
                    Some(LocalHarnessSetupState::MissingHarness { tooltip }) => {
                        localized_validation_message(tooltip, ctx_dropdown)
                    }
                    Some(LocalHarnessSetupState::ProductDisabled { message }) => {
                        localized_validation_message(message, ctx_dropdown)
                    }
...
pub fn render_mode_toggle<A: OrchestrationControlAction>(
    ...
    let local_segment = render_segment_button::<A>(
        &localization::text_for_app(app, "agent.orchestration.controls.local"),
        !is_remote,
    ...
    let cloud_segment = render_segment_button::<A>(
        &localization::text_for_app(app, "agent.orchestration.controls.cloud"),
        is_remote,
...
pub fn render_picker_row_with_layout<A: OrchestrationControlAction>(
    ...
    let api_key_label = localization::text_for_app(app, "agent.orchestration.controls.api_key");
    ...
    let base_model_label =
        localization::text_for_app(app, "agent.orchestration.controls.base_model");
