...
impl RunPlatform {
    ...
    /// Builds the same summary using the application's active locale.
    pub fn summary_for_app(&self, app: &AppContext) -> String {
        let platform = match self {
            RunPlatform::Runner(platform) => platform,
            RunPlatform::Default => {
                return format!(
                    "{} · {}",
                    localization::text_for_app(app, "agent_sdk.runner.value.os.linux"),
                    localization::text_for_app(app, "agent_sdk.runner.value.arch.x86_64"),
                );
            }
        };
        ...
        let os_label = platform
            .macos_version
            .map(|version| {
                let version = macos_version_number_display(version);
            ...
            .unwrap_or_else(|| {
                let key = match platform.os {
                    RunnerOs::Linux => "agent_sdk.runner.value.os.linux",
                    RunnerOs::Macos => "agent_sdk.runner.value.os.macos",
                };
                localization::text_for_app(app, key)
            });

        let arch_key = match platform.arch {
            RunnerArch::X8664 => "agent_sdk.runner.value.arch.x86_64",
            RunnerArch::Aarch64 => "agent_sdk.runner.value.arch.aarch64",
        };
        let mut parts = vec![os_label, localization::text_for_app(app, arch_key)];
        if !platform.name.is_empty() {
            parts.push(platform.name.clone());
        }
        if let Some(vcpus) = platform.vcpus {
            let vcpus = vcpus.to_string();
            parts.push(localization::text_for_app_with_args(
                app,
                "agent_sdk.runner.value.vcpus",
                &[("count", &vcpus)],
            ));
        }
        if let Some(memory_gb) = platform.memory_gb {
            let memory_gb = memory_gb.to_string();
            parts.push(localization::text_for_app_with_args(
                app,
                "agent_sdk.runner.value.memory_gb",
                &[("count", &memory_gb)],
            ));
        }
        parts.join(" · ")
...
fn macos_version_number_display(version: RunnerMacOsVersion) -> &'static str {
    match version {
        RunnerMacOsVersion::Macos14 => "14",
        RunnerMacOsVersion::Macos15 => "15",
        RunnerMacOsVersion::Macos26 => "26",
        RunnerMacOsVersion::Macos27 => "27",
    }
}
