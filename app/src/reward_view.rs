...
impl RewardView {
    ...
    fn subtitle(&self, app: &AppContext) -> String {
        match self.kind {
            RewardKind::SentReferralTheme => {
                localization::text_for_app(app, "reward.subtitle.sent_referral")
            }
            RewardKind::ReceivedReferralTheme => {
                localization::text_for_app(app, "reward.subtitle.received_referral")
            }
        }
    }
    ...
    fn render_title(&self, ui_builder: &UiBuilder, app: &AppContext) -> Box<dyn Element> {
        Align::new(
            ui_builder
                .span(localization::text_for_app(app, "reward.title"))
                .with_style(UiComponentStyles {
...
impl View for RewardView {
    ...
    fn accessibility_contents(&self, app: &AppContext) -> Option<AccessibilityContent> {
        Some(AccessibilityContent::new(
            format!(
                "{} {}",
                localization::text_for_app(app, "reward.title"),
                self.subtitle(app)
            ),
            localization::text_for_app(app, "reward.a11y.help"),
