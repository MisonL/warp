...
#[test]
fn test_tab_config_menu_disambiguates_by_localized_name() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        ...
        app.read(|ctx| {
            let tab_configs = vec![
                crate::tab_configs::TabConfig {
                    ...
                    name_zh_cn: Some("同名配置".to_string()),
                    title: None,
                    color: None,
                    panes: Vec::new(),
                    params: HashMap::new(),
            ...
            assert_eq!(
                disambiguated_tab_config_display_names(&tab_configs, ctx),
                vec!["同名配置".to_string(), "同名配置 (1)".to_string()]
            );
        });
    });
}
