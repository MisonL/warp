...
impl AuthViewBody {
    ...
    fn render_sign_in_row(&self, ui_builder: &UiBuilder, app: &AppContext) -> Box<dyn Element> {
        Flex::row()
            .with_child(
                ui_builder
                    .span(localization::text_for_app(app, "auth.already_have_account"))
                    .build()
                    .finish(),
            )
            .with_child(
                ui_builder
                    .link(
                        localization::text_for_app(app, "auth.sign_in"),
                        None,
                        Some(Box::new(|ctx| {
        ...
        Container::new(
            Flex::row()
                ...
                .with_child(
                    ui_builder
                        .link(
                            localization::text_for_app(app, "auth.skip_for_now"),
                            None,
                            Some(Box::new(|ctx| {
    ...
    fn render_browser_open_content(
        ...
        let auth_token = Container::new(match self.render_auth_token_input(appearance) {
            ...
            _ => self.render_auth_token_suggest(ui_builder, app),
        })
        .with_margin_top(AUTH_MODAL_GAP)
        .finish();
