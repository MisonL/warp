...
/// flight.
const GENERATING_PLACEHOLDER_KEY: &str = "code_review.git_dialog.commit.generating_message";
...
const FALLBACK_PLACEHOLDER_KEY: &str = "code_review.git_dialog.commit.type_message";
...
/// ran.
const LOADING_LABEL_KEY: &str = "code_review.git_dialog.commit.committing";
...
pub(super) fn new_state(
    ...
    let (push_label_key, push_icon) = if has_upstream {
        (
    ...
    let initial_placeholder_key = if ai_autogen_enabled {
        GENERATING_PLACEHOLDER_KEY
    } else {
    ...
    let message_editor = ctx.add_typed_action_view(|ctx| {
        ...
        editor.set_placeholder_text(
            localization::text_for_app(ctx, initial_placeholder_key),
            ctx,
        );
        editor
    });
    ...
    let commit_and_push_button = ctx.add_typed_action_view(move |ctx| {
        ActionButton::new(
            localization::text_for_app(ctx, push_label_key),
            SecondaryTheme,
        )
        .with_size(ButtonSize::XSmall)
        .with_height(32.)
    ...
    let state = CommitState {
        ...
        message_editor,
        placeholder_key: initial_placeholder_key,
        push_label_key,
        commit_button,
...
pub(super) fn loading_label(app: &AppContext) -> String {
    localization::text_for_app(app, LOADING_LABEL_KEY)
}
...
pub(super) fn apply_generated_commit_message(
    ...
    let editor_handle = match me.mode_mut() {
        GitDialogMode::Commit(state) => {
            state.placeholder_key = FALLBACK_PLACEHOLDER_KEY;
            state.message_editor.clone()
        }
        _ => return,
    };
    match result {
        Ok(generated) => {
            ...
            editor_handle.update(ctx, |editor, ctx| {
                ...
                editor.set_placeholder_text(
                    localization::text_for_app(ctx, FALLBACK_PLACEHOLDER_KEY),
                    ctx,
                );
...
pub(super) fn start_confirm(me: &mut GitDialog, ctx: &mut ViewContext<GitDialog>) {
    ...
    me.set_loading(localization::text_for_app(ctx, LOADING_LABEL_KEY), ctx);
...
pub(super) fn finish_commit_chain(
    ...
    match &result {
        ...
        Ok(None) => {
            let key = if matches!(intent, CommitChainMode::CommitOnly) {
                "code_review.git_dialog.commit.committed"
            } else {
                "code_review.git_dialog.commit.committed_and_pushed"
            };
            show_toast(localization::text_for_app(ctx, key), ctx);
        }
        Err(err) => {
...
pub(super) fn refresh_localized_text(state: &CommitState, ctx: &mut ViewContext<GitDialog>) {
    state.message_editor.update(ctx, |editor, ctx| {
        editor.set_placeholder_text(localization::text_for_app(ctx, state.placeholder_key), ctx);
    });
    state.commit_button.update(ctx, |button, ctx| {
        button.set_label(
            localization::text_for_app(ctx, "code_review.git.commit"),
            ctx,
        );
    });
    state.commit_and_push_button.update(ctx, |button, ctx| {
        button.set_label(localization::text_for_app(ctx, state.push_label_key), ctx);
    });
    if let Some(button) = &state.commit_and_create_pr_button {
