    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
            ...
            .is_inline_history_menu()
        {
            ...
            let message = InlineHistoryMessageProducer
                .produce_message(InlineHistoryMessageArgs { selected, app })
                .unwrap_or_default();
    ...
    fn produce_message(&self, args: InlineHistoryMessageArgs<'_>) -> Option<Message> {
        ...
        let items = match args.selected {
            ...
            Some(AcceptHistoryItem::Conversation { title, .. }) => {
                vec![
                    enter,
                    MessageItem::text(localization::text_for_app_with_args(
                        args.app,
                        "terminal.message_bar.to_open",
                        &[("title", title)],
                    )),
                ]
            }
...
impl MessageTransformer<TerminalMessageArgs<'_>> for AttachedTextSelectionMessageTransformer {
    fn transform_message(&self, message: &mut Message, args: TerminalMessageArgs<'_>) -> bool {
        ...
        message.append_text(&localization::text_for_app(
            args.app,
