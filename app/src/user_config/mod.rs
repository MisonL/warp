...
#[cfg(feature = "local_fs")]
pub(crate) fn ensure_default_worktree_config(locale: LocaleId) -> PathBuf {
    ...
    if !path.exists() {
        ...
        let template = default_worktree_template_for_locale(locale);
...
#[cfg(feature = "local_fs")]
pub(crate) fn default_worktree_template_for_locale(locale: LocaleId) -> &'static str {
    match locale {
        LocaleId::EnUs => include_str!("../../resources/tab_configs/default_worktree.toml"),
        LocaleId::ZhCn => include_str!("../../resources/tab_configs/default_worktree.zh-CN.toml"),
    }
}
...
pub(crate) fn new_tab_config_template_for_locale(locale: LocaleId) -> &'static str {
    match locale {
        ...
        LocaleId::ZhCn => {
            include_str!("../../resources/tab_configs/new_tab_config_template.zh-CN.toml")
        }
    }
}
...
pub(crate) fn materialize_default_worktree_config(
    ...
    if let Some(doc) = toml_value.as_table_mut() {
        let template_name = doc
            ...
            .map(str::to_string);
        let template_name_zh_cn = doc
            .get("name_zh_CN")
            .and_then(toml::Value::as_str)
            .map(str::to_string);
        let canonical_name = default_worktree_canonical_config_name(
            config_name,
            &repo_display_name,
            template_name.as_deref(),
            template_name_zh_cn.as_deref(),
        );
        doc.insert(
            "name".to_string(),
            toml::Value::String(canonical_name.clone()),
        );
        if let Some(name_zh_cn) = localized_default_worktree_config_name(
            config_name,
            &canonical_name,
            template_name.as_deref(),
            template_name_zh_cn.as_deref(),
        ) {
            doc.insert("name_zh_CN".to_string(), toml::Value::String(name_zh_cn));
        } else {
            doc.remove("name_zh_CN");
        }
    }
...
#[cfg(feature = "local_fs")]
fn default_worktree_canonical_config_name(
    config_name: &str,
    repo_display_name: &str,
    template_name: Option<&str>,
    template_name_zh_cn: Option<&str>,
) -> String {
...
#[cfg(feature = "local_fs")]
fn localized_default_worktree_config_name(
    config_name: &str,
    canonical_name: &str,
    template_name: Option<&str>,
    template_name_zh_cn: Option<&str>,
) -> Option<String> {
    if let (Some(template_name), Some(template_name_zh_cn)) = (template_name, template_name_zh_cn) {
        if has_worktree_template_prefix(config_name, template_name_zh_cn) {
            return Some(config_name.to_string());
        }
        if let Some(suffix) = canonical_name.strip_prefix(template_name) {
            return Some(format!("{}{}", template_name_zh_cn, suffix));
        }
    }
...
#[cfg(feature = "local_fs")]
fn has_worktree_template_prefix(config_name: &str, template_name: &str) -> bool {
