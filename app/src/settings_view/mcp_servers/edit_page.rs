...
impl MCPServersEditPageView {
    ...
    fn refresh_localized_text(&mut self, ctx: &mut ViewContext<Self>) {
        for (button, key) in [
            (&self.save_button, "settings.action.save"),
            (&self.reinstall_button, "settings.mcp.edit.edit_variables"),
            (&self.delete_button, "settings.mcp.edit.delete_mcp"),
            (&self.unshare_button, "settings.mcp.edit.remove_from_team"),
        ] {
            button.update(ctx, |button, ctx| {
                button.set_label(localization::text_for_app(ctx, key), ctx);
            });
        }
        self.editing_disabled_banner.update(ctx, |banner, ctx| {
            banner.set_content(
    ...
    fn detect_secrets_in_templatable_mcp_server(
        ...
        if should_block_save_for_secrets(safe_mode_enabled, enterprise_enforced, contains_secrets) {
            let error = localization::text_for_app(ctx, "settings.mcp.edit.error.contains_secrets");
    ...
    fn build_templatable_mcp_server_result_from_json(
        ...
        if parsed_templatable_mcp_servers.len() > 1 {
            let error = localization::text_for_app(
                ctx,
