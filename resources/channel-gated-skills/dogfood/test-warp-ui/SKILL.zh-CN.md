...
`--api-key` 标志会使用 `STAGING_USER_WARP_API_KEY` 环境变量中的 API 密钥认证，因此应用会直接启动，不需要交互式登录。
...
如果只需要验证某个 UI 外观，可以临时硬编码或模拟数据，让目标 UI 状态无需完整导航流程即可到达。此步骤是可选的；测试应自然工作的端到端流程时跳过。

适合硬编码的例子：

- **条件 UI**：功能只在特定条件下出现，例如某个设置、非空数据集、活跃订阅。可硬编码条件让 UI 始终出现。
- **功能标志**：功能在尚未启用的标志后面。可直接启用。
- **错误状态**：需要测试错误处理 UI。可硬编码错误响应或失败条件。

模拟改动必须最小且聚焦，只改到达待测 UI 状态所必需的内容。
...
- **任务描述要具体**：不要说“检查对话框是否正确”，而是说“打开设置，点击常规标签页，并描述第一个区域的文本和布局”。
