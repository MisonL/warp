...
description_zh_CN: "**必需前置技能**：每次调用 `use_figma` 工具前都必须先调用此技能；绝不能在未加载此技能时直接调用 `use_figma`，否则会产生常见且难以调试的失败。用户要在 Figma 文件上下文中执行需要 JavaScript 的写入操作或特殊读取操作时触发，例如创建、编辑或删除节点，设置变量或 token，构建组件和变体，修改 auto-layout 或填充，绑定变量到属性，或以编程方式检查文件结构。"
