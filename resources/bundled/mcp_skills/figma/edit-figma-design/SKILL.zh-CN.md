...
description_zh_CN: 使用 Figma MCP 创作工具，根据文字产品或 UI 描述直接创建或更新 Figma 设计。适用于从文本生成 mockup、线框、页面、组件、流程或概念设计，或根据文字反馈迭代现有 Figma 文件。需要 Figma MCP 服务器连接。
metadata:
  mcp-server: figma
---
...
本技能用于根据自然语言描述直接创建或更新 Figma 设计。它结合 Figma library search 和直接文件编辑；只有当设计需要贴合产品或代码库时，才使用 Warp agent 的额外上下文能力。

## 适用场景

当用户想要：

- 根据文字描述在 Figma 中设计新页面、流程、组件或 mockup。
- 根据文字反馈修改或扩展现有 Figma 文件。
- 直接在 Figma 中创建第一版 wireframe 或更高保真设计。
- 让 Figma 设计对齐现有 design system 或产品词汇。

不要用于：

- 从设计生成生产代码，改用 `implement-design`。
- 把运行中的页面或 app capture 到 Figma，改用 `figma-generate-design`。
...
- 收集最少必要信息：要设计什么、使用现有文件还是新文件、是否要贴合 design system 或代码库。
...
### 第 1 步：确认这是 Figma authoring 请求

如果用户实际要实现代码，停止并改用 `implement-design`。

如果用户要 screenshot-to-Figma 或网页 capture，停止并改用 `figma-generate-design`。本技能用于 text-to-design authoring。
...
当用户只要探索性概念、wireframe 或 mockup，且没有要求对齐代码库时，只使用 Figma MCP。

当用户要求对齐现有产品或 design system 时，选择性使用 Warp agent 上下文：

- 读取 `AGENTS.md` 或 `WARP.md` 中的项目规则。
- 使用语义搜索、grep 和文件读取寻找相关组件、产品词汇、布局模式和 design token 来源。
- 只有用户请求直接依赖外部系统或灵感来源时，才使用其他 MCP 或 web search。
- 本技能正常流程中不要编辑代码、运行 REPL 或使用 computer use。

### 第 4 步：authoring 前搜索 design system
...
新文件：用户要求为桌面 app 设计 billing overview screen，并使用现有 design system。先确认是 Figma authoring，然后用 `whoami` 和 `create_new_file` 确定目标文件，必要时读取项目规则，调用 `search_design_system` 搜索 billing 相关组件，再用小步 `use_figma` 构建页面，最后返回文件 URL。

更新现有文件：用户提供 Figma URL 并要求添加 onboarding checklist。提取 `fileKey`，搜索 checklist、card、badge、progress 资产，用增量 `use_figma` 添加新 section，返回同一 URL 并说明变更。

纯探索概念：用户要 mobile workout planner mockup，且不需要匹配代码库。新建文件，跳过代码库搜索，只用 Figma library 资产，直接构建概念并询问下一步修订。
...
- 用户同时要求 Figma 设计和实现：若主要请求是 Figma authoring，先创建设计；若主要请求是实现，改用 `implement-design`。设计确认后可另起实现步骤。
