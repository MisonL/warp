...
#[test]
fn config_rejects_narrow_variant_with_missing_or_extra_keys() {
    let mut config = config(vec![tab(2, "secondary")]);
    config.main_tab = Some(tab(1, "main"));
    let mut variant = config.clone();
    variant.main_tab = None;
    variant.tabs = vec![tab(3, "replacement")];
    config.narrow_variants = vec![TuiTabBarNarrowVariant {
        max_width_exclusive: 80,
        config: variant,
    }];

    assert_eq!(
        TuiTabBarView::new(config).err(),
        Some(TuiTabBarConfigError::NarrowVariantKeysMismatch {
            max_width_exclusive: 80,
            missing: vec![key(1), key(2)],
            extra: vec![key(3)],
        })
    );
}
