...
use warp::tui_export::{
    ...
    OrchestrationEventStreamer, OrchestrationEventStreamerEvent, PrepareRemoteChildLaunchError,
    PreparedRemoteChildLaunch, RemoteChildLaunchConfig, RenderableAIError, ServerApiProvider,
    StartAgentExecutionMode, StartAgentRequest, aggregated_orchestrator_status,
...
/// Returns the localized main-tab label used for the orchestration tree root.
pub(crate) fn orchestrator_tab_label() -> String {
    localization::text("tui.orchestration.tab_bar.orchestrator")
}

/// Returns the root tab label for a specific locale without changing the
/// process-wide TUI locale.
#[cfg(test)]
pub(crate) fn orchestrator_tab_label_for_locale(locale: LocaleId) -> String {
    localization::text_for_locale(locale, "tui.orchestration.tab_bar.orchestrator")
}
...
pub(crate) struct TuiOrchestrationSnapshot {
    ...
    /// Main-tab label: the anchor's agent name, or the localized orchestrator
    /// label when the anchor is the tree root.
...
impl TuiOrchestrationModel {
    ...
    /// deepest-first, so no descendant session is orphaned. The presentation
    /// flag does not affect cleanup because restored history can still contain
    /// nested sessions while multi-level orchestration is disabled.
