...
pub enum TuiTabBarConfigError {
    /// A stable key was assigned to more than one tab in the rendered row.
    DuplicateKey(String),
    ...
    /// A narrow variant does not preserve the base row's tab identities.
    NarrowVariantKeysMismatch {
        max_width_exclusive: u16,
        missing: Vec<String>,
        extra: Vec<String>,
    },
}
...
impl fmt::Display for TuiTabBarConfigError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NarrowVariantKeysMismatch {
                max_width_exclusive,
                missing,
                extra,
            } => write!(
                formatter,
                "TUI tab-bar narrow variant below {max_width_exclusive} columns has mismatched \
...
fn validated_live_keys(config: &TuiTabBarConfig) -> Result<HashSet<String>, TuiTabBarConfigError> {
    let base_keys = validated_config_keys(config)?;
    let mut live_keys = base_keys.clone();
    for variant in &config.narrow_variants {
        let variant_keys = validated_config_keys(&variant.config)?;
        let mut missing = base_keys
            .difference(&variant_keys)
            .cloned()
            .collect::<Vec<_>>();
        let mut extra = variant_keys
        ...
        if !missing.is_empty() || !extra.is_empty() {
            return Err(TuiTabBarConfigError::NarrowVariantKeysMismatch {
                max_width_exclusive: variant.max_width_exclusive,
                missing,
                extra,
            });
        }
        live_keys.extend(variant_keys);
    }
    Ok(live_keys)
}

fn validated_config_keys(
    config: &TuiTabBarConfig,
) -> Result<HashSet<String>, TuiTabBarConfigError> {
