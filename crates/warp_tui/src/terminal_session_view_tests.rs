...
fn shell_completion_source_warmup_loads_path_executables() {
    App::test((), |mut app| async move {
        let fixture = focus_test_fixture(&mut app);
        let (view, _) = add_focus_test_session(&mut app, &fixture, true);
        let command_dir = TempDir::new().expect("command fixture directory");
        let command_name = "warp-test-command";
        #[cfg(windows)]
        let command_path = command_dir.path().join(format!("{command_name}.exe"));
        #[cfg(not(windows))]
        let command_path = command_dir.path().join(command_name);
        #[cfg(windows)]
        {
            let comspec = std::env::var_os("ComSpec").expect("ComSpec should be set on Windows");
            fs::copy(comspec, &command_path).expect("copy Windows command fixture");
        }
        #[cfg(not(windows))]
        fs::write(&command_path, "exit 0\n").expect("write command fixture");
        #[cfg(unix)]
        {
        ...
        let test_path = std::env::join_paths(
            std::env::split_paths(&inherited_path)
                ...
                .chain(std::iter::once(command_dir.path().as_os_str().to_owned())),
        )
        .expect("construct test PATH")
        .to_string_lossy()
        .into_owned();
        let session = Arc::new(Session::test_with_path(Some(test_path)));
        ...
        assert!(
            session
                .executable_names()
                .any(|command| command == command_name),
            "external command fixture should be available for completion"
        );
    });
}
