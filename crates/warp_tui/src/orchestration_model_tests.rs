...
use super::{
    TuiOrchestrationModel, orchestrator_tab_label_for_locale,
...
fn restoring_parent_materializes_supported_descendant_sessions() {
    ...
    App::test((), |mut app| async move {
        ...
        app.read(|ctx| {
            ...
            assert_eq!(
                snapshot.anchor_label,
                orchestrator_tab_label_for_locale(LocaleId::EnUs)
            );
        ...
        app.read(|ctx| {
            ...
            assert_eq!(
                snapshot
                    .breadcrumbs
                    .iter()
                    .map(|breadcrumb| (breadcrumb.conversation_id, breadcrumb.label.clone()))
                    .collect::<Vec<_>>(),
                vec![(
                    parent_conversation_id,
                    orchestrator_tab_label_for_locale(LocaleId::EnUs),
        ...
        app.read(|ctx| {
            ...
            assert_eq!(
                snapshot
                    ...
                    .collect::<Vec<_>>(),
                vec![
                    (root_id, orchestrator_tab_label_for_locale(LocaleId::EnUs)),
...
fn kill_child_agent_subtree_removes_nested_descendants_with_the_child() {
    assert_kill_child_agent_subtree_removes_nested_descendants(true);
}

#[test]
fn kill_child_agent_subtree_cleans_nested_descendants_when_multi_level_disabled() {
    assert_kill_child_agent_subtree_removes_nested_descendants(false);
}

fn assert_kill_child_agent_subtree_removes_nested_descendants(multi_level_enabled: bool) {
    let _flag = FeatureFlag::MultiLevelOrchestration.override_enabled(multi_level_enabled);
    App::test((), |mut app| async move {
        let fixture = orchestration_fixture(&mut app);
        let parent_session_id = add_dispatching_session(&mut app, &fixture, true);
        let parent_conversation_id = app.read(|ctx| {
